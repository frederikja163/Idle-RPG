    ...
    CleanupEventListener
{
  ...
  public async getByItemId(profileId: ProfileId, itemId: ItemId) {
    try {
      if (!this.inventoryCache.hasInventory(profileId))
      ...
      console.error(
        `Failed to get item ${itemId} for profile ${profileId}`,
        error
      );
  ...
  public updateInventory(profileId: ProfileId) {
    try {
      if (this.dirtyItems.hasKey(profileId))
        this.dirtyItems.removeKey(profileId);
      this.dirtyInventories.add(profileId);
    } catch (error) {
      console.error(`Failed marking inventory as dirty ${profileId}`, error);
    }
  }
  ...
  public updateItem(profileId: ProfileId, itemId: ItemId) {
    try {
      ...
      console.error(
        `Failed marking item as dirty ${profileId} ${itemId}`,
        error
      );
    }
  }
  ...
  public async onProfileSelected({
    ...
    try {
      ...
      console.error(
        `Failed warming up inventory cache on profile select ${profileId}`,
        error
      );
    }
  }
  public onProfileDeselected({
    ...
    try {
      ...
      console.error(
        `Failed invalidating cache on profile deselect ${profileId}`,
        error
      );
    }
  }
    ...
    try {
      ...
      await this.db.transaction(async (tx) => {
        ...
        for (const profileId of dirtyInventories) {
          ...
          if (items)
            await this.inventoryRepo.updateItems(
              profileId,
