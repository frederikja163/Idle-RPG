...
export class ItemService implements ProfileSelectedEventListener, ProfileDeselectedEventListener, CleanupEventListener {
  ...
  public async getItemsByProfileId(profileId: ProfileId) {
    const cache = this.itemCache.getItemsByProfileId(profileId)?.values().toArray();
  ...
  public async cleanup(): Promise<void> {
    try {
      ...
      this.profilesToRemove.forEach(this.itemCache.invalidateProfile.bind(this.itemCache));
