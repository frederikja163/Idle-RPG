    ...
    CleanupEventListener
{
  ...
  public async getOrCreateByGoogle(
    googleId: string,
    email: string,
    ...
    const user =
      ...
      (await this.createGoogleUser(googleId, email, profilePicture));
    if (!user)
      throw new ServerError(ErrorType.InternalError, "Couldn't register user.");
    return user;
  }
  ...
  private async createGoogleUser(
    ...
    return await this.db.transaction(
      async (tx) =>
        await this.userRepo.create({ googleId, email, profilePicture }, tx)
    );
  }

  public async deleteUser(userId: UserId) {
    await this.db.transaction(
      async (tx) => await this.userRepo.delete(userId, tx)
    );
  }
  ...
  public async cleanup(): Promise<void> {
    try {
      ...
      await this.db.transaction(
        async (tx) => await this.userRepo.updateTimes(userIds, tx)
      );
    } catch (error) {
