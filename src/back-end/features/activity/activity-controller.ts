import type { SocketOpenEventData, SocketOpenEventListener } from '@/back-end/core/events/socket-event';
...
export class ActivityController implements SocketOpenEventListener {
  ...
  private async handleStartActivity(socket: ServerSocket, { activityId }: ServerData<'Activity/StartActivity'>) {
    ...
    if (profile.activityId && profile.activityStart)
      if (!this.stopActivity(profileId, activity, profile.activityStart)) return socket.error(ErrorType.InternalError);
  ...
  private handleGetActivity(socket: ServerSocket, {}: ServerData<'Activity/GetActivity'>) {}

  private async stopActivity(profileId: ProfileId, activity: Activity, activityStart: Date) {
    const actionCount = getActionCount(activityStart, activity.time);
