  ...
  implements ProfileSelectedEventListener, ProfileDeselectedEventListener, CleanupEventListener
{
  ...
  public async getSkillsByProfileId(profileId: ProfileId): Promise<Skill[]> {
    const cache = this.skillCache.getSkillsByProfileId(profileId)?.values().toArray();
  ...
  public async getSkillById(profileId: ProfileId, id: SkillId) {
    ...
    const cache = this.skillCache.getSkillById(profileId, id);
    if (cache) return cache;
    const skill = { profileId, id, xp: 0, level: 0 };
    this.skillCache.store(skill);
    return skill;
  }

  public update(profileId: ProfileId, id: SkillId) {
    this.dirtySkills.add(profileId, id);
  }

  public updateMany(profileId: ProfileId, ids: SkillId[]) {
  ...
  public async cleanup(): Promise<void> {
    try {
      ...
      this.profilesToRemove.forEach(this.skillCache.invalidateCache.bind(this.skillCache));
